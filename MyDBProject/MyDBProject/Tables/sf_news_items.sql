CREATE TABLE [dbo].[sf_news_items] (
    ...
    CONSTRAINT [pk_sf_news_items] PRIMARY KEY CLUSTERED ([content_id] ASC) WITH (FILLFACTOR = 90)
);


GO
CREATE NONCLUSTERED INDEX [idx_sf_news_org_cnt_id]
    ON [dbo].[sf_news_items]([original_content_id] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idx_sf_news]
    ON [dbo].[sf_news_items]([app_name] ASC) WITH (FILLFACTOR = 90);
