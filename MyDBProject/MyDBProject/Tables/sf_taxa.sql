CREATE TABLE [dbo].[sf_taxa] (
    ...
    CONSTRAINT [pk_sf_taxa] PRIMARY KEY CLUSTERED ([id] ASC) WITH (FILLFACTOR = 90)
);


GO
CREATE NONCLUSTERED INDEX [idx_sf_taxa_fct_txn_fct_tx_id]
    ON [dbo].[sf_taxa]([fct_txn_fct_tx_id] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idx_sf_taxa_parent_id]
    ON [dbo].[sf_taxa]([parent_id] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idx_sf_taxa_taxonomy_id]
    ON [dbo].[sf_taxa]([taxonomy_id] ASC) WITH (FILLFACTOR = 90);
