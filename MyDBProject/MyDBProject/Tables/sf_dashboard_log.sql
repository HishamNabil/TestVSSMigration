CREATE TABLE [dbo].[sf_dashboard_log] (
    ...
    CONSTRAINT [pk_sf_dashboard_log] PRIMARY KEY CLUSTERED ([id] ASC) WITH (FILLFACTOR = 90)
);


GO
CREATE NONCLUSTERED INDEX [idx_sf_dashboard_log_time]
    ON [dbo].[sf_dashboard_log]([timestamp] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idx_sf_dashboard_log_id_type]
    ON [dbo].[sf_dashboard_log]([item_id] ASC, [item_type] ASC) WITH (FILLFACTOR = 90);
