CREATE TABLE [dbo].[sf_cs_threads] (
    ...
    CONSTRAINT [pk_sf_cs_threads] PRIMARY KEY CLUSTERED ([id] ASC) WITH (FILLFACTOR = 90)
);


GO
CREATE UNIQUE NONCLUSTERED INDEX [idx_sf_cs_threads_key]
    ON [dbo].[sf_cs_threads]([resolve_key] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idx_sf_cs_threads_group]
    ON [dbo].[sf_cs_threads]([parent_group_id] ASC) WITH (FILLFACTOR = 90);
