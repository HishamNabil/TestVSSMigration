CREATE TABLE [dbo].[sf_user_profile_link] (
    ...
    CONSTRAINT [pk_sf_user_profile_link] PRIMARY KEY CLUSTERED ([id] ASC) WITH (FILLFACTOR = 90)
);


GO
CREATE UNIQUE NONCLUSTERED INDEX [UC_profile]
    ON [dbo].[sf_user_profile_link]([profile_id] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idx_sf_profile_link_user_id]
    ON [dbo].[sf_user_profile_link]([user_id] ASC) WITH (FILLFACTOR = 90);
