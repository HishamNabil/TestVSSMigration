CREATE TABLE [dbo].[sf_content_relation] (
    ...
    CONSTRAINT [pk_sf_content_relation] PRIMARY KEY CLUSTERED ([key] ASC) WITH (FILLFACTOR = 90)
);


GO
CREATE NONCLUSTERED INDEX [idx_sf_cntnt_rltn_subj]
    ON [dbo].[sf_content_relation]([subject_id] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idx_sf_cntnt_rltn_obj]
    ON [dbo].[sf_content_relation]([object_id] ASC) WITH (FILLFACTOR = 90);
