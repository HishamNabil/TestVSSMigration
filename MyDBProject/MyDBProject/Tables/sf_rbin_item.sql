CREATE TABLE [dbo].[sf_rbin_item] (
    ...
    CONSTRAINT [pk_sf_rbin_item] PRIMARY KEY CLUSTERED ([id] ASC) WITH (FILLFACTOR = 90)
);


GO
CREATE NONCLUSTERED INDEX [idx_sf_rbin_item_type_prov_id]
    ON [dbo].[sf_rbin_item]([del_item_type_name] ASC, [del_item_prvdr_name] ASC, [del_item_id] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idx_sf_rbin_date_created]
    ON [dbo].[sf_rbin_item]([date_created] ASC) WITH (FILLFACTOR = 90);
