CREATE TABLE [dbo].[sf_media_file_urls] (
    ...
    CONSTRAINT [pk_sf_media_file_urls] PRIMARY KEY CLUSTERED ([id] ASC) WITH (FILLFACTOR = 90)
);


GO
CREATE UNIQUE NONCLUSTERED INDEX [sf_idx_mfu_url_mfl]
    ON [dbo].[sf_media_file_urls]([url] ASC, [media_file_link_id] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idx_sf_md_fl_rls_md_fl_link_id]
    ON [dbo].[sf_media_file_urls]([media_file_link_id] ASC) WITH (FILLFACTOR = 90);
