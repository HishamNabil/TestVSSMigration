CREATE TABLE [dbo].[sf_meta_fields] (
    ...
    CONSTRAINT [pk_sf_meta_fields] PRIMARY KEY CLUSTERED ([id] ASC) WITH (FILLFACTOR = 90)
);


GO
CREATE NONCLUSTERED INDEX [idx_sf_meta_fields_type_id]
    ON [dbo].[sf_meta_fields]([type_id] ASC) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [idx_sf_metafields]
    ON [dbo].[sf_meta_fields]([app_name] ASC, [field_name] ASC) WITH (FILLFACTOR = 90);
